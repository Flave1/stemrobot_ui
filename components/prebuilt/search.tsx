...
import { InternetSearchProps } from "../types/tools";

export function InternetSearchLoading(): JSX.Element {
  return (
...
export function InternetSearch(props: InternetSearchProps): JSX.Element {
  ...
  return (
    <Card className="w-[325px] max-w-[325px] p-4 h-[300px] max-h-[300px] flex flex-col text-gray-50 bg-black">
      ...
      <div className="flex-grow flex flex-col justify-center items-center mb-8">
        <div>
          {props.result.map((element: any) => {
            return (
              <div key={element.ur}>
                <p className="text-[96px] font-bold">{element.url}</p>
                <p className="text-[96px] font-bold">{element.content}</p>
              </div>
            );
          })}
        </div>
